...
// Redis key for WhatsApp messages
const WHATSAPP_MESSAGES_KEY = "lemonde:whatsapp_messages"
...
export async function GET(req: NextRequest) {
  try {
    ...
    const messages = (await redis.get<WhatsAppMessage[]>(WHATSAPP_MESSAGES_KEY)) || []
    ...
    return NextResponse.json({ error: "Erro ao buscar mensagens do WhatsApp" }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.messageId && body.reply) {
      ...
      // Update Redis
      await redis.set(WHATSAPP_MESSAGES_KEY, messages)
    ...
    if (body.mockMessage && session.user.role === "admin") {
      const newMessage: WhatsAppMessage = {
        ...
        sender: {
          id: nanoid(),
          name: body.mockMessage.name || "Cliente WhatsApp",
          phoneNumber: body.mockMessage.phoneNumber || "+5567999999999",
